...
export default function Feed() {
  ...
  // URL-dan parametrlarni olish (selectedTopic va searchQuery)
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // API uchun query parametrlarini yasash
      let postsUrl = `${API_BASE}/posts?limit=20`;
      if (currentCategory !== 'All') {
        // Backend ID kutayotgan bo'lsa, kategoriyani topamiz
        const catObj = categories.find(c => c.name === currentCategory);
        if (catObj) postsUrl += `&category=${catObj._id}`;
      }
      if (currentSearch) postsUrl += `&search=${currentSearch}`;
      ...
      if (userRes.status === 'fulfilled') setCurrentUser(userRes.value.data.data || userRes.value.data);
      ...
      <div className="flex-1 lg:ml-[280px]">
        ...
        <header className="sticky top-0 bg-white/80 backdrop-blur-md z-40 border-b border-gray-50 px-6 py-4">
          <div className="max-w-4xl mx-auto flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden">☰</button>
            ...
            <Link href="/create-post" className="bg-black text-white px-6 py-3 rounded-2xl font-black text-sm hover:bg-gray-800 transition shadow-lg shadow-black/10">
              WRITE
            </Link>
          </div>
        </header>
                  ...
                  <div className="grid md:grid-cols-12 gap-8">
                    <div className="md:col-span-8">
                      <Link href={`/posts/${post._id}`}>
                        <h2 className="text-2xl font-black mb-3 leading-tight group-hover:underline underline-offset-4 decoration-2">{post.title}</h2>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`flex items-center space-x-2 px-4 py-2 rounded-2xl transition ${isLiked ? 'bg-pink-50 text-pink-600' : 'hover:bg-gray-50 text-gray-400'}`}
                        >
                    ...
                    <div className="md:col-span-4 hidden md:block">
                      <div className="aspect-square bg-gray-50 rounded-[32px] border border-gray-100 flex items-center justify-center text-gray-200 text-4xl">
                        🖼️
                      </div>
                    </div>
                  </div>
