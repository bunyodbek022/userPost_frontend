...
export default function PostsAnalytics() {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            // Fetch all for chart, and paginated for table
            const [chartRes, tableRes] = await Promise.all([
                api.get('/posts?limit=5000&status=ALL'), // All posts
                api.get('/posts', {
                    params: {
                        page,
                        limit: 10,
            ...
            const allPosts = chartRes.data.data || chartRes.data || [];
    ...
    const handleDeletePost = async (id: string, title: string) => {
        if (!confirm(`"${title}" sarlavhali maqolani o'chirmoqchimisiz?`)) return;
        try {
    ...
    const filteredPostsCount = chartData.reduce((a, b) => a + b, 0);
        ...
        <>
            <div className="max-w-[1200px] mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10 animate-fade-up">
                        <div>
                            <h1 className="font-serif text-3xl md:text-4xl font-black tracking-tight mb-2 text-[#16120E] dark:text-white">Maqolalar</h1>
                            <p className="text-gray-500 dark:text-gray-400 font-bold text-[10px] sm:text-xs tracking-wide uppercase">Barcha maqolalar ro'yxati va tahlili</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#1E293B] rounded-[24px] p-8 border border-gray-100 dark:border-white/5 shadow-sm mb-10 animate-fade-up [animation-delay:100ms]">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400 mb-6">Maqolalar Filtrlari</h3>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-gray-50/50 dark:bg-white/5">
                                    <tr>
                                        <th className="px-8 py-5 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Maqola</th>
                                        <th className="px-8 py-5 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400">Muallif</th>
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                    {loading && postsList.length === 0 ? (
                                        <tr>
                                            <td colSpan={4} className="px-8 py-20 text-center text-gray-400 font-bold text-sm">
                                        ...
                                        <tr>
                                            <td colSpan={4} className="px-8 py-20 text-center text-gray-400 font-bold text-[13px] uppercase tracking-wide">Maqolalar topilmadi</td>
                                        </tr>
                                    ) : (
                                        postsList.map((post) => (
                                            <tr key={post._id} className="group hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                                <td className="px-8 py-5">
                                                    <div className="font-bold text-sm text-[#16120E] dark:text-gray-200 group-hover:text-[#e8440a] transition-colors truncate max-w-xs">{post.title}</div>
                                                </td>
                                                <td className="px-8 py-5">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-[10px] font-black text-gray-600 dark:text-gray-400 shadow-sm">
                                                            {post.author?.userName?.charAt(0).toUpperCase()}
                                                        </div>
                                                        <span className="text-gray-500 font-medium text-xs">{post.author?.userName}</span>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-5">
